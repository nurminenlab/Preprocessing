...
spkraster_L_tmp   = NaN*ones(N_units, nStims/2, nTrials, 700);
spkraster_NoL_tmp = NaN*ones(N_units, nStims/2, nTrials, 700);
    ...
    for f = 1:nStims/2
        ...
        for g = 1:length(idx)
            ...
            if Laser_Mat(idx(g)) == 1
                ...
                spkraster_L_tmp(u,f,g,:) = histcounts(spk_tmp,[-raster_trail:1:stimParams.on_time_ms+raster_trail]);
                % count
                ...
                spkraster_NoL_tmp(u,f,g,:) = histcounts(spk_tmp,[-raster_trail:1:stimParams.on_time_ms+raster_trail]);
                % count
